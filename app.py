...
# BIS PUBLIC VIEW
    ...
    if df_full is not None:
        ...
        for label, klassen, geschl in bis_defs:
            ...
            # Liste der Richter, die in dieser speziellen Klasse abgestimmt haben
            abgestimmte_richter = [
                key.replace(prefix, "") 
            ...
            for i, j in enumerate(judges):
                with r_cols[i+1]:
                    # Hintergrundfarbe: Grün bei Vote, aber nur wenn der Sieger noch NICHT enthüllt wurde
                    is_voted = j in abgestimmte_richter
                    bg_color = "#28a745" if (is_voted and not winner_revealed) else "#1a4a9e"
                    ...
                    if show_noms:
                        ...
                        if not m.empty:
                            ...
                            st.markdown(f"""
                                <div class='cat-card' style='background-color: {bg_color};'>
                            ...
                            st.markdown(f"<div class='placeholder-box' style='background-color: {bg_color};'>–</div>", unsafe_allow_html=True)
                    else: 
