            ...
            with r_cols[-1]:
                if winner_revealed:
                    ...
                    if winner_nr and winner_nr != "Automatisch (Stimmen)":
                        ...
                        if not m_w.empty: st.markdown(f"<div class='cat-card bis-winner-card'><div class='cat-number'>{winner_nr}</div><div class='cat-details'>{get_full_label(m_w.iloc[0])}</div></div>", unsafe_allow_html=True)
                else: st.markdown("<div class='placeholder-box'>🔒</div>", unsafe_allow_html=True)
