...
# --- 1. SETUP & STYLING (RÜCKGÄNGIG AUF DAS ORIGINAL-DESIGN) ---
st.set_page_config(layout="wide", page_title="KECB Burgdorf 2026", page_icon="🐾")

st.markdown("""
    <style>
    /* Das ursprüngliche, große Display-Design */
...
def check_auth(role, password):
    ...
    if not st.session_state[auth_key]:
        with st.container():
            ...
            pwd_input = st.text_input(f"Passwort für {role}", type="password", key=f"p_{role}")
...
st.sidebar.markdown("---")
view = st.sidebar.radio("Navigation:", ["📢 Dashboard", "🏆 BIS Grid", "Steward", "Richter", "Admin"])
