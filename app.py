...
# BIS PUBLIC VIEW NEW
    ...
    if df_full is not None:
        ...
        # --- KATZEN-ZEILEN ---
        # +++ NEU: Aktive Klasse exakt abrufen (inklusive Kategorie und Tag) +++
        active_live_label = store.data.get("test_live_label")
        active_live_cat = store.data.get("test_live_cat")
        active_live_tag = store.data.get("test_live_tag")

        for label, klassen, geschl in bis_defs:
            r_cols = st.columns([0.8] + [1.2]*len(judges) + [0.8])
            
            # +++ NEU: Strenge Prüfung (muss für exakt diese Ansicht aktiv sein) +++
            is_active = (label == active_live_label and sel_cat == active_live_cat and tag == active_live_tag)
            active_style = "background-color: #d9d2d2 !important; border-color: #b0a8a8 !important;" if is_active else ""
            
            # +++ GEÄNDERT: style='{active_style}' hinzugefügt +++
                ...
                with r_cols[i+1]:
                    if show_noms:
                        ...
                        if not m.empty:
                            ...
                            # +++ GEÄNDERT: f-string und style='{active_style}' hinzugefügt +++
