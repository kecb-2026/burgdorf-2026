...
st.markdown("""
    ...
    .tag-biv, .steward-biv { background-color: #28a745 !important; color: white !important; }
    .tag-nom, .steward-nom { background-color: #ffc107 !important; color: black !important; }
    
    /* NEU: Kontrastreiche Box pro Katze im Steward-Modul */
    .steward-item {
        padding: 15px;
        border-radius: 12px;
        border: 1px solid #ccc;
        margin-bottom: 10px;
        background-color: #f8f9fa; /* Hellgrau für Standard-Kontrast */
    }
    </style>
    ...
    if st.session_state.view == "Dashboard":
        ...
        if mein_richter != "--":
            ...
            for _, row in df_j.iterrows():
                ...
                # Dynamische Hintergrundfarbe für die Steward-Zeile
                bg_style = ""
                if store.data[k]["BIV"]: bg_style = "steward-biv"
                elif store.data[k]["NOM"]: bg_style = "steward-nom"

                st.markdown(f'<div class="steward-item {bg_style}">', unsafe_allow_html=True)
