...
if st.session_state.view == "Home":
    ...
    st.subheader("Willkommen! Bitte wählen Sie Ihren Zugang:")
    
    col1, col2 = st.columns(2)
    with col1:
        if st.button("📢 LIVE-DASHBOARD\n(Öffentlich)"): set_view("Dashboard")
        if st.button("🏆 BEST IN SHOW GRID\n(Öffentlich)"): set_view("BIS_Grid")
    with col2:
        if st.button("📝 STEWARD-PULT\n(Login)"): set_view("Steward_Login")
        if st.button("👨‍⚖️ RICHTER / ⚙️ ADMIN\n(Login)"): set_view("Admin_Login")
...
# --- LOGIN SCREENS ---
    ...
    if st.button("Anmelden"):
        if pwd == "steward2026": set_view("Steward_Panel")
