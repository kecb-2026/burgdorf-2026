...
rows_def = [
    ...
    {"label": "Junior 11", "f": lambda r: str(r.get('Klasse','')) == '11'},
    {"label": "Kitten 12", "f": lambda r: str(r.get('Klasse','')) == '12'}
]
...
if st.session_state.view == "Home":
    ...
    df_full = load_labels()
    r_col = f"Richter {tag}"
    df_active = df_full[df_full[tag].astype(str).str.upper() == 'X'].copy() if df_full is not None else None
    ...
    if st.session_state.view == "Dashboard":
                    ...
                    for k, v in store.data.items():
                        ...
                        if r_n == j and any(v.values()):
                            ...
                            if not m.empty:
                                ...
                                card_html = f"""
                                <div class='cat-card'>
                                    <div style='font-size:20px; font-weight:bold; color:#d9534f;'>Kategorie {row['Kategorie']}</div>
                ...
                for j in judges:
                    ...
                    for k, v in store.data.items():
                        if v.get("NOM"):
                            nr, r_n = k.split("|")
                            if r_n == j:
                                match = df_active[df_active['KAT_STR'] == nr]
        ...
        if st.button("🔴 DATEN LÖSCHEN"): store.data = {}; st.rerun()
        st.write(store.data)
