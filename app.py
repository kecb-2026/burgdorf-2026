...
st.markdown("""
    ...
    /* Steward Label Box */
    .steward-label-box {
        background-color: #1a4a9e; color: white; padding: 10px; border-radius: 10px;
        font-weight: bold; font-size: 20px; width: 100%; text-align: left;
    }
    </style>
...
if st.session_state.view == "Home":
                ...
                for k, v in store.data.items():
                    ...
                    if r_n == j and any(v.values()):
                        ...
                        if not m.empty:
                            ...
                            card_html = f"""
                            <div class='cat-card'>
                                <div style='font-size:18px; font-weight:bold;'>Kategorie {row['Kategorie']}</div>
    ...
    if st.sidebar.button("🏠 Home"): set_view("Home")
