...
# --- EIGENSTÄNDIGE VIEW: NOMINATION LABELS DRUCK ---
    ...
    if df_full is not None:
        # Nur Katzen filtern mit Nominierungs-X (Dein Original-Filter)
        ...
        if not df_nominierte.empty:
            ...
            # DEINE PDF-FUNKTION (KOMPLETT UNBERÜHRT UND ORIGINAL)
            ...
            # SAFE VORSCHAU KONFIGURATION (Aus deinem Original)
            ...
            # Automatische Erkennung der Tages-Spalte in deinen Daten (z.B. 'TAG' oder 'ANMELDETAG')
            tag_col = next((c for c in df_nominierte.columns if c.upper() in ['TAG', 'DAY', 'ANMELDETAG', 'AUSSTELLUNGSTAG']), None)
            ...
            with tab_tag1:
                if tag_col:
                    df_t1 = df_nominierte[df_nominierte[tag_col].astype(str).str.contains('1|Samstag|Sam|Sa', case=False, na=False)].copy()
                else:
                    df_t1 = df_nominierte.copy()

                st.info(f"Aktuell sind **{len(df_t1)}** Katzen für den Labeldruck an Tag 1 bereit.")
                
                pdf_labels_t1 = generate_avery_labels(df_t1)
                st.download_button(
                    label="📥 Avery Zweckform PDF generieren & herunterladen (Tag 1)",
                    data=pdf_labels_t1,
                    file_name="KECB_Nomination_Labels_Tag1.pdf",
                    mime="application/pdf",
                    key="dl_btn_t1"
                )
                st.write("### Vorschau der enthaltenen Katzen (Tag 1):")
                st.dataframe(df_t1[verfuegbare_spalten], column_config=aktuelle_config, use_container_width=True, hide_index=True, key="preview_table_t1")

            # --- TAB 2 (SONNTAG) ---
            with tab_tag2:
                if tag_col:
                    df_t2 = df_nominierte[df_nominierte[tag_col].astype(str).str.contains('2|Sonntag|Son|So', case=False, na=False)].copy()
                else:
                    df_t2 = df_nominierte.copy()

                st.info(f"Aktuell sind **{len(df_t2)}** Katzen für den Labeldruck an Tag 2 bereit.")
                
                pdf_labels_t2 = generate_avery_labels(df_t2)
                st.download_button(
                    label="📥 Avery Zweckform PDF generieren & herunterladen (Tag 2)",
                    data=pdf_labels_t2,
                    file_name="KECB_Nomination_Labels_Tag2.pdf",
                    mime="application/pdf",
                    key="dl_btn_t2"
                )
                st.write("### Vorschau der enthaltenen Katzen (Tag 2):")
                st.dataframe(df_t2[verfuegbare_spalten], column_config=aktuelle_config, use_container_width=True, hide_index=True, key="preview_table_t2")
